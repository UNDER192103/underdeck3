...
let notifyMainRendererReady = null;
let updateRestartScheduled = false;
let loadingRendererReady = false;
let notifyLoadingRendererReady = null;
...
const ipcmainService = new IpcmainService(AppService, expressService, hortcutService, fileDialogService, themeService, soundPadService, obsService, webDeckService, updaterService, async () => { await syncOverlayShortcut(); }, () => { trayService.refreshMenu(); }, async (windowsSettings) => { await systemStartupService.syncWithSettings(Boolean(windowsSettings.autoStart)); }, async () => {
    ensureLoadingWindow();
...
const createMainApplicationWindow = () => {
    ...
    const tryShowMainWindow = () => {
        if (mainReadyReceived)
            return;
        if (!readyToShow || !cssReady || !mainRendererReady)
            return;
...
const registerUpdateLifecycle = () => {
    updaterService.on("loading-state-changed", (state) => emitLoadingState(state));
    ...
    updaterService.on("debug-log", (payload) => {
        if (String(process.env.UPDATER_DEBUG_BROADCAST || "").trim() !== "1")
            return;
        BrowserWindow.getAllWindows().forEach((win) => {
            if (win.isDestroyed())
