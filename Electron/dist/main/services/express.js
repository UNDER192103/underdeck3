...
export class ExpressServer {
    ...
    constructor(port, appService, webDeckService, soundPadService, obsService) {
        this.app = express();
        this.port = port;
    ...
    resolveMediaRelativePathToAbsolutePath(relativePath) {
    ...
    getAutoItemKey(type, refId) {
        return `${type}:${refId}`;
    }
    mapPageMediaUrls(page, autoPageIcons, autoItemIcons) {
        ...
        const items = page.items.map((item) => {
            ...
            const fallbackAutoPageIcon = itemType === "page" && itemRefId.startsWith("__auto_page_")
                ? autoPageIcons?.[itemRefId] ?? null
                : null;
            const autoItemKey = itemType === "soundpad" || itemType === "obs" || itemType === "app"
                ? this.getAutoItemKey(itemType, itemRefId)
                : "";
    ...
    mapAutoIconsMediaUrls(autoIcons) {
        const pages = {};
        const items = {};
    ...
    getThemePreferences() {
      ...
      `).run();
            const themeRow = db.prepare("SELECT value FROM theme_preferences WHERE key = ? LIMIT 1").get("theme");
            ...
            const background = (() => {
                if (!backgroundRow?.value)
                    return defaults.background;
                try {
                    const parsed = JSON.parse(backgroundRow.value);
    ...
    setupStaticRoutes() {
        ...
        const webdeckIndexFromDir = (dir) => path.join(dir, "webdeck", "index.html");
        const webdeckHtmlFromDir = (dir) => path.join(dir, "webdeck.html");
        const loadingIndexFromDir = (dir) => path.join(dir, "loading", "index.html");
        ...
        if (!staticDir) {
            ...
            this.app.get("/", (_req, res) => {
                res.status(200).json({
                    ...
                    hint: "Defina WEBDECK_STATIC_DIR ou copie o build do cliente para resources/webdeck-client para servir a UI.",
                });
            });
            return;
        }
