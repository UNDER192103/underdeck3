...
export class MainAppService extends EventEmitter {
    ...
    soundPadService;
    obsService;
    constructor(soundPadService, obsService) {
        super();
    ...
    ensureStorageFolder(folderName) {
        const folder = path.join(this.getStorageRootPath(), folderName);
        fs.mkdirSync(folder, { recursive: true });
        return folder;
    }
    getStorageDirectoryPath(folderName) {
    ...
    importFileToStorage(sourcePath, folderName, targetFileName) {
    ...
    resolveIconToAbsolutePath(icon) {
        if (!icon)
            return null;
    ...
    deleteIconFileIfUnreferenced(icon) {
        if (!icon)
            return;
        if (this.hasIconReference(icon))
            return;
        this.deleteIconFileIfLocal(icon);
    }
    persistEntityIcon(icon, entityId, folderName) {
        if (!icon)
            return null;
        if (icon.startsWith("underdeck-media://"))
            return icon;
        ...
        const imported = this.importFileToStorage(iconPath, folderName, `${entityId}${extension.toLowerCase()}`);
    ...
    getAppDataBase() {
        ...
        if (!columns.some((column) => column.name === "position")) {
            ...
            const rows = db.prepare("SELECT id FROM apps ORDER BY rowid ASC").all();
    ...
    addApp(app) {
        const db = this.getAppDataBase();
        const maxPositionRow = db.prepare("SELECT MAX(position) as maxPosition FROM apps").get();
        const appData = {
            ...app,
        ...
        db.prepare('INSERT INTO apps (id, position, type, name, icon, banner, description, meta_data) VALUES (?, ?, ?, ?, ?, ?, ?, ?)').run(appData.id, appData.position, appData.type, appData.name, appData.icon, appData.banner, appData.description, JSON.stringify(appData.meta_data));
    ...
    updateApp(app) {
        ...
        const db = this.getAppDataBase();
        db.prepare('UPDATE apps SET position = ?, type = ?, name = ?, icon = ?, banner = ?, description = ?, meta_data = ? WHERE id = ?').run(appData.position, appData.type, appData.name, appData.icon, appData.banner, appData.description, JSON.stringify(appData.meta_data), appData.id);
    ...
    deleteApp(id) {
        ...
        const shortcutDb = this.getShortcutDataBase();
        const shortcutRows = shortcutDb.prepare('SELECT * FROM shortcuts').all();
        shortcutRows.forEach((row) => {
            try {
                const metaData = JSON.parse(row.meta_data ?? "{}");
                if (metaData?.appId !== id)
                    return;
    ...
    async executeApp(id) {
        ...
        switch (app.type) {
            ...
            case 3: {
                if (!("action" in app.meta_data) && !("type" in app.meta_data))
                    return false;
            ...
            case 5: {
                ...
                if (!connected)
                    return false;
                const meta = app.meta_data;
                const target = String(meta?.target ?? "").toLowerCase();
                const action = String(meta?.action ?? "").toLowerCase();
                if (target === "stream") {
                    if (action === "start")
                        return (await this.obsService.startStream()).ok;
                    if (action === "stop")
                        return (await this.obsService.stopStream()).ok;
                    if (action === "toggle")
                        return (await this.obsService.toggleStream()).ok;
                    return false;
                }
                if (target === "record") {
                    if (action === "start")
                        return (await this.obsService.startRecord()).ok;
                    if (action === "stop")
                        return (await this.obsService.stopRecord()).ok;
                    if (action === "toggle")
                        return (await this.obsService.toggleRecordPause()).ok;
                    if (action === "pause")
                        return (await this.obsService.pauseRecord()).ok;
                    if (action === "resume")
                        return (await this.obsService.resumeRecord()).ok;
                    return false;
                }
                if (target === "scene") {
                ...
                if (target === "audio") {
                    ...
                    if (!inputName)
                        return false;
                    if (action === "mute")
                        return (await this.obsService.setInputMute(inputName, true)).ok;
                    if (action === "unmute")
                        return (await this.obsService.setInputMute(inputName, false)).ok;
                    if (action === "toggle")
                        return (await this.obsService.toggleInputMute(inputName)).ok;
                    return false;
                }
                ...
                if (legacyType === "action") {
                    ...
                    if (normalized.includes("startstream"))
                        return (await this.obsService.startStream()).ok;
                    if (normalized.includes("stopstream"))
                        return (await this.obsService.stopStream()).ok;
                    if (normalized.includes("togglestream"))
                        return (await this.obsService.toggleStream()).ok;
                    if (normalized.includes("startrecord"))
                        return (await this.obsService.startRecord()).ok;
                    if (normalized.includes("stoprecord"))
                        return (await this.obsService.stopRecord()).ok;
                    if (normalized.includes("togglerecordpause"))
                        return (await this.obsService.toggleRecordPause()).ok;
                    if (normalized.includes("pauserecord"))
                        return (await this.obsService.pauseRecord()).ok;
                    if (normalized.includes("resumerecord"))
                        return (await this.obsService.resumeRecord()).ok;
                }
                return false;
            }
    ...
    repositionApp(id, toPosition) {
        const db = this.getAppDataBase();
        const rows = db.prepare("SELECT id FROM apps ORDER BY position ASC, rowid ASC").all();
    ...
    addShortcut(shortcut) {
        ...
        const db = this.getShortcutDataBase();
        const result = db.prepare('INSERT INTO shortcuts (id, type, name, icon, banner, description, meta_data) VALUES (?, ?, ?, ?, ?, ?, ?)').run(shortcutData.id, shortcutData.type, shortcutData.name, shortcutData.icon, shortcutData.banner, shortcutData.description, JSON.stringify(shortcutData.meta_data));
    ...
    updateShortcut(shortcut) {
        ...
        const db = this.getShortcutDataBase();
        db.prepare("UPDATE shortcuts SET type = ?, name = ?, icon = ?, banner = ?, description = ?, meta_data = ? WHERE id = ?").run(shortcutData.type, shortcutData.name, shortcutData.icon, shortcutData.banner, shortcutData.description, JSON.stringify(shortcutData.meta_data), shortcutData.id);
