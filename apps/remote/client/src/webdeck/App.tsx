...
function WebDeckRemoteAppContent() {
  ...
  const autoPages = useMemo<WebDeckViewPage[]>(() => {
    ...
    return [
      ...buildAutoPagedPages({
        ...
        backLabel: t("remote.webdeck.back", "Back"),
        nextLabel: t("remote.webdeck.next", "Next"),
      }),
      ...buildAutoPagedPages({
  ...
  }, [config.pages, config.soundpad.audios, config.obs.scenes, config.obs.audioInputs, config.apps, autoItemIcons, autoPageIcons, t]);
  ...
  const getItemLabel = (item: WebDeckItem | null | undefined) => {
    ...
    return appMap.get(item.refId)?.name || t("remote.webdeck.item", "Item");
  };

  if (loading) {
    return (
      <div className="min-h-screen w-full flex items-center justify-center bg-black text-white">
        {t("remote.webdeck.app.loading", "Loading...")}
      </div>
    );
  }
  ...
  if (!currentPage) {
    return (
      <div className="min-h-screen w-full flex items-center justify-center bg-black text-white">
        {t("remote.webdeck.app.no_pages", "No pages available.")}
      </div>
    );
  }
