...
import { AppUser, FriendUser, UserTag } from "@/types/user";
...
function renderTagIcon(tag: UserTag) {
  ...
  if (isUrlIcon(icon)) {
    return (
      <img src={icon} alt="" className="h-4.5 w-4.5 rounded-sm object-cover" />
    );
  }
...
export function ProfilePreviewCard({
  ...
  return (
    ...
    >
      <div
        className={`relative ${size === "dropdown" ? "h-30" : "h-36"} w-full rounded-t-lg overflow-hidden`}
      >
        {bannerUrl ? (
          <Img
            src={bannerUrl}
      ...
      <div className="relative flex-1 min-h-0 bg-black/20 rounded-b-lg">
        ...
        >
          <p
            className="text-base text-xl font-bold text-white cursor-pointer"
            onClick={onMoreUserInfoClick}
          >
            {user.displayName}
          </p>
