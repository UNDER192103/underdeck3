...
async function getProcessedGifAvatarFile(
  baseFile: File,
  cropArea: Area,
): Promise<File> {
...
export function AvatarEditorModal({
  ...
  const handleSave = async () => {
    ...
    try {
      if (isGif) {
        const processedGif = await getProcessedGifAvatarFile(
          selectedFile,
          croppedAreaPixels,
        );
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[620px] select-none rounded-xl bg-background/50 backdrop-blur supports-[backdrop-filter]:bg-background/80">
        <DialogHeader>
          <DialogTitle>
            {t("user.avatar.edit_title", "Editar avatar")}
          </DialogTitle>
        </DialogHeader>
            ...
            <div className="space-y-3">
              <div className="mx-auto h-56 w-56 overflow-hidden rounded-full border border-border bg-muted">
                <img
                  ...
                  alt={t("user.avatar.current_alt", "Avatar atual")}
            ...
            <>
              ...
              <div className="flex flex-col gap-2">
                <Label>
                  {t(
                    "user.crop.drag_zoom",
                    "Arraste para mover e ajuste o zoom",
                  )}
                </Label>
                <input
                  type="range"
