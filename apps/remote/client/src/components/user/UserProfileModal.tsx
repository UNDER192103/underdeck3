...
export function UserProfileModal({ isOpen, onClose }: UserProfileModalProps) {
  ...
  const acceptFriendRequest = async (requestId: string) => {
    try {
      ...
      toast.error(
        t("remote.friends.accept.fail", "Failed to accept request."),
        {
          description: error?.response?.data?.error || error?.message || "",
        },
      );
  ...
  const declineFriendRequest = async (requestId: string) => {
    try {
      ...
      toast.error(
        t("remote.friends.decline.fail", "Failed to decline request."),
        {
          description: error?.response?.data?.error || error?.message || "",
        },
      );
  ...
  return (
    <>
      <Dialog open={isOpen} onOpenChange={onClose}>
        ...
        >
          <Card className="flex flex-row rounded-xl bg-card/70 p-3">
            <ProfilePreviewCard
              className="w-[70%]"
              user={previewUser}
              noteEditable={true}
            ...
            />
            <Card className="h-full w-full overflow-auto bg-card/70 p-2">
              ...
              <Tabs
                value={friendsTab}
                onValueChange={(value) => setFriendsTab(value as any)}
              >
                <TabsList className="w-full">
                  <TabsTrigger
                    variant={
                      friendsTab === "accepted" ? "primary" : "secondary"
                    }
                    value="accepted"
                  >
                  ...
                  <TabsTrigger
                    variant={
                      friendsTab === "incoming" ? "primary" : "secondary"
                    }
                    value="incoming"
                  >
                  ...
                  <TabsTrigger
                    variant={
                      friendsTab === "outgoing" ? "primary" : "secondary"
                    }
                    value="outgoing"
                  >
                        ...
                        <div className="flex items-center justify-between gap-2 rounded-lg border border-white/10 bg-black/20 p-2 text-xs text-white/60">
                          <DropdownUp>
                            <DropdownUpTrigger asChild>
                              <div className="h-full text-lg flex items-center gap-1 cursor-pointer">
                                <img
                                  src={
                                    friendIncoming.fromUser.avatarUrl ||
                                    defaultAvatar
                                  }
                                  alt={
                                    friendIncoming.fromUser.displayName ||
                                    friendIncoming.fromUser.username
                                  }
                                  className="w-9 h-9 rounded-full border-2 border-primary cursor-pointer"
                                />
                                {friendIncoming.fromUser.displayName ||
                          ...
                          <div className="flex items-center gap-2">
                            <Button
                              ...
                              rounded="xl"
                              onClick={() =>
                                acceptFriendRequest(friendIncoming.id)
                              }
                            >
                            ...
                            <Button
                              ...
                              rounded="xl"
                              onClick={() =>
                                declineFriendRequest(friendIncoming.id)
                              }
                            >
                              <X className="h-4 w-4" />
                          ...
                          <DropdownUp>
                            <DropdownUpTrigger asChild>
                              <div className="w-full h-full text-lg flex items-center gap-1 cursor-pointer">
                                <img
                                  src={
                                    friendOutgoing.toUser.avatarUrl ||
                                    defaultAvatar
                                  }
                                  alt={
                                    friendOutgoing.toUser.displayName ||
                                    friendOutgoing.toUser.username
                                  }
                                  className="w-9 h-9 rounded-full border-2 border-primary cursor-pointer"
                                />
                                {friendOutgoing.toUser.displayName ||
      ...
      <Dialog open={addFriendOpen} onOpenChange={setAddFriendOpen}>
        <DialogContent className="more-dark sm:max-w-[420px]">
          <DialogHeader>
            <DialogTitle>{t("remote.friends.send", "Send")}</DialogTitle>
          </DialogHeader>
          <div className="grid gap-2">
            <Label>
              {t("remote.friends.identifier.placeholder", "username or id")}
            </Label>
            <Input
              rounded="xl"
              ...
              placeholder={t(
                "remote.friends.identifier.placeholder",
                "username or id",
              )}
