...
export default function OverlayDeckView() {
  ...
  const addCacheBuster = (url: string | null | undefined): string => {
    if (!url) return "";
  ...
  const resolveItemBackground = (item: WebDeckItem | null | undefined) => {
    ...
    if (item.icon) {
      ...
      iconUrl = pageMap.get(item.refId)?.icon || autoPageIcons[item.refId] || null;
    } else if (item.type === "back") {
      const target = item.refId || history[history.length - 1] || "";
      iconUrl = target ? pageMap.get(target)?.icon || null : null;
    } else {
      iconUrl = appMap.get(item.refId)?.icon || null;
    }
