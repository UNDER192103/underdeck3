...
export function UserModalLogin() {
  const { login, register, forgotPassword, options, setOptions } = useUser();
  const { t } = useI18n();
      ...
      <DialogContent className="sm:max-w-[525px] select-none rounded-xl bg-background/50 backdrop-blur supports-[backdrop-filter]:bg-background/50">
        ...
        {mode === "login" ? (
          <div className="flex flex-col gap-4 py-4">
            ...
            >
              {t("auth.forgot", "Esqueceu a senha?")}
            </button>
          </div>
        ) : mode === "register" ? (
          <div className="flex flex-col gap-4 py-4">
        ...
        <DialogFooter className="sm:justify-center">
          {mode === "login" ? (
            ...
            <Button className="w-full h-10" variant="primary" rounded="xl" onClick={tryForgotPassword}>
              <Send size={20} /> {t("auth.forgot.send", "Enviar email")}
            </Button>
          )}
        </DialogFooter>
      </DialogContent>
