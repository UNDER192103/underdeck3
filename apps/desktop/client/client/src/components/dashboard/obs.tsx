...
export default function ObsStudio({
  ...
  const [audioVolumeMap, setAudioVolumeMap] = useState<Record<string, number>>({});
  ...
  useEffect(() => {
    if (!obsState?.audioInputs) return;
    const next: Record<string, number> = {};
    for (const input of obsState.audioInputs) {
      const db = Number.isFinite(input.inputVolumeDb) ? input.inputVolumeDb : 0;
      next[input.inputName] = Math.min(0, Math.max(-100, db));
    }
    setAudioVolumeMap(next);
  }, [obsState?.audioInputs]);
                ...
                {pagedAudios.length === 0 ? (
                  ...
                  pagedAudios.map((input: ObsAudioInput) => (
                    <Card key={input.inputUuid || input.inputName} className="p-3 border-border/70 bg-card/70">
                      {(() => {
                        const volumeDb = audioVolumeMap[input.inputName] ?? 0;
                        const formattedDb = Number.isFinite(volumeDb) ? volumeDb.toFixed(2) : "0.00";
                        return (
                          <>
                            <div className="flex items-center justify-between gap-2">
                              <div className="min-w-0">
                            ...
                            <div className="mt-3 grid gap-2">
                              <div className="flex items-center justify-between">
                                <Label className="text-xs text-muted-foreground">{t("obs.audio.mixer", "Mixer")}</Label>
                                <span className="text-xs text-muted-foreground">{formattedDb} dB</span>
                              </div>
                              <Slider
                                value={[volumeDb]}
                                min={-100}
                                max={0}
                                step={0.01}
                                disabled={!input.canSetVolume || !!busyMap[`audio-volume-${input.inputName}`]}
                                onValueChange={(values) => {
                                  const dbValue = Number(values?.[0] ?? 0);
                                  setAudioVolumeMap((prev) => ({
                                    ...prev,
                                    [input.inputName]: Math.min(0, Math.max(-100, dbValue)),
                                  }));
                                }}
                                onValueCommit={(values) => {
                                  const dbValue = Number(values?.[0] ?? 0);
                                  const volume = Math.min(0, Math.max(-100, dbValue));
                                  void runObsAction(
                                    `audio-volume-${input.inputName}`,
                                    () => window.underdeck.obs.setInputVolume(input.inputName, volume)
                                  );
                                }}
                              />
                            </div>
