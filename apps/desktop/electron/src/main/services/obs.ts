...
export class ObsService extends EventEmitter {
    ...
    private bindSocketEvents() {
        ...
        this.socket.on("InputVolumeChanged", (event: any) => {
            const inputName = String(event?.inputName ?? "");
            if (!inputName) return;
            const inputVolumeDb = Number(event?.inputVolumeDb ?? 0);
            const inputVolumeMul = Number(event?.inputVolumeMul ?? 0);

            let found = false;
            this.audioInputs = this.audioInputs.map((input) => {
                if (input.inputName !== inputName) return input;
                found = true;
                return { ...input, inputVolumeDb, inputVolumeMul };
            });
    ...
    private async getAudioInputs() {
        ...
        try {
            ...
            const mapped: Omit<ObsAudioInput, "inputMuted" | "inputVolumeDb" | "inputVolumeMul" | "canSetVolume">[] = rawInputs.map((input: any) => ({
            ...
            const settled = await Promise.allSettled(
                mapped.map(async (input) => {
                    const mutePromise = this.socket.call("GetInputMute", { inputName: input.inputName });
                    const volumePromise = this.socket.call("GetInputVolume", { inputName: input.inputName });
                    const [mute, volume] = await Promise.all([mutePromise, volumePromise]);
                    return {
                        key: input.inputName,
                        muted: Boolean((mute as any)?.inputMuted),
                        volumeDb: Number((volume as any)?.inputVolumeDb ?? 0),
                        volumeMul: Number((volume as any)?.inputVolumeMul ?? 0),
                        canSetVolume: true,
                    };
                })
            );

            const resultMap = new Map<string, { muted: boolean; volumeDb: number; volumeMul: number, canSetVolume: boolean }>();
            for (const result of settled) {
                if (result.status === "fulfilled") {
                    resultMap.set(result.value.key, {
                        muted: result.value.muted,
                        volumeDb: result.value.volumeDb,
                        volumeMul: result.value.volumeMul,
                        canSetVolume: result.value.canSetVolume,
                    });
                }
            }
            ...
            this.audioInputs = mapped.map((item) => ({
                ...item,
                inputMuted: resultMap.get(item.inputName)?.muted ?? false,
                inputVolumeDb: resultMap.get(item.inputName)?.volumeDb ?? 0,
                inputVolumeMul: resultMap.get(item.inputName)?.volumeMul ?? 0,
                canSetVolume: resultMap.get(item.inputName)?.canSetVolume ?? false,
            }));
        } catch {
            this.audioInputs = [];
        }
    ...
    public async connect(config?: Partial<ObsResolvedConfig>): Promise<ObsCommandResult> {
        ...
        try {
            ...
            this.lastError = null;
            await this.refreshState();
            logsService.log("obs", "connect.success", { host: finalConfig.host, port: finalConfig.port });
            ...
            await this.emitStateChanged();
            logsService.log("obs", "connect.error", { error: this.lastError }, "error");
    ...
    public async disconnect(): Promise<ObsCommandResult> {
        ...
        this.currentProgramSceneName = "";
        await this.emitStateChanged();
        logsService.log("obs", "disconnect");
    ...
    private async callWhenConnected(
        ...
        if (!connected) {
            logsService.log("obs", "command.error", { message: this.lastError ?? "Nao foi possivel conectar ao OBS." }, "error");
            return { ok: false, message: this.lastError ?? "Nao foi possivel conectar ao OBS." };
        }
        try {
            await callback();
            await this.refreshState();
            logsService.log("obs", "command.success", { message: successMessage });
            return { ok: true, message: successMessage };
        } catch (error) {
            this.lastError = this.normalizeError(error);
            await this.emitStateChanged();
            logsService.log("obs", "command.error", { message: this.lastError }, "error");
            return { ok: false, message: this.lastError };
        }
    }
    ...
    public async setInputVolume(inputNameOrUuid: string, inputVolumeDb: number) {
        const normalized = String(inputNameOrUuid ?? "").trim();
        if (!normalized) {
            return { ok: false, message: "Input de audio invalido." };
        }
        const volume = Number(inputVolumeDb);
        if (!Number.isFinite(volume) || volume < -100 || volume > 0) {
            return { ok: false, message: "Volume invalido." };
        }
        const input = this.findInput(normalized);
        const inputName = input?.inputName || normalized;
        return this.callWhenConnected(async () => {
            await this.socket.call("SetInputVolume", { inputName, inputVolumeDb: volume });
        }, "Volume do audio alterado.");
    }
