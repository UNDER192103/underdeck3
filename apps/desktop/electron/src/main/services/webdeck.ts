...
export type WebDeckAutoIcons = {
    pages: Record<string, string>;
    items: Record<string, string>;
};

export class WebDeckService extends EventEmitter {
    private static readonly DEFAULT_COLS = 5;
    ...
    private normalizeIconSource(iconSource: string | null | undefined) {
        if (!iconSource) return null;
        const value = String(iconSource).trim();
        return value || null;
    }

    private savePageIcon(pageId: string, iconSource: string | null | undefined) {
        const source = this.normalizeIconSource(iconSource);
        if (!source) return null;
        if (source.startsWith("underdeck-media://")) return source;
    ...
    private deleteIconIfLocal(icon: string | null | undefined) {
        if (!icon) return;
        if (!icon.startsWith("underdeck-media://")) return;
    ...
    private hasItemIconReference(icon: string | null | undefined) {
        if (!icon) return false;
        const db = this.getDatabase();
        const rows = db.prepare("SELECT items_json FROM webdeck_pages").all() as Array<{ items_json: string }>;
        for (const row of rows) {
            let items: Array<WebDeckItem | null> = [];
            try {
    ...
    private deleteItemIconIfUnreferenced(icon: string | null | undefined) {
        if (!icon) return;
        if (this.hasItemIconReference(icon)) return;
        this.deleteIconIfLocal(icon);
    }

    private saveAutoIcon(kind: "page" | "item", iconKey: string, iconSource: string | null | undefined) {
        const source = this.normalizeIconSource(iconSource);
        if (!source) return null;
        if (source.startsWith("underdeck-media://")) return source;
    ...
    private hasAutoIconReference(kind: "page" | "item", icon: string | null | undefined) {
        ...
        const row = db
            ...
            .get(kind, icon) as { total: number } | undefined;
        return Number(row?.total ?? 0) > 0;
    }

    private deleteAutoIconIfUnreferenced(kind: "page" | "item", icon: string | null | undefined) {
        if (!icon) return;
        if (this.hasAutoIconReference(kind, icon)) return;
        this.deleteIconIfLocal(icon);
    }

    private safeGridValue(value: number, fallback: number) {
        if (!Number.isFinite(value)) return fallback;
        const safe = Math.trunc(value);
        if (safe < 2) return fallback;
        return Math.min(safe, 20);
    }

    private normalizeItems(rawItems: unknown, slots: number) {
        const items = Array.isArray(rawItems) ? rawItems : [];
        const normalized: Array<WebDeckItem | null> = new Array(slots).fill(null);
    ...
    private mapRowToPage(row: any): WebDeckPage {
        ...
        return {
            id: String(row.id),
            name: String(row.name ?? "Nova Página"),
    ...
    private persistPage(page: WebDeckPage) {
        const db = this.getDatabase();
        db.prepare(`
    ...
    private hasBackItem(page: WebDeckPage) {
        return page.items.some((item) => item?.type === "back");
    }

    private removeBackItems(page: WebDeckPage) {
        const nextItems = page.items.map((item) => (item?.type === "back" ? null : item));
        return { ...page, items: nextItems };
    }

    private ensureBackItem(page: WebDeckPage): WebDeckPage {
        if (this.hasBackItem(page)) return page;
    ...
    private normalizeBackReference(page: WebDeckPage, firstPageId: string | null, isFirstPage: boolean) {
        if (isFirstPage) return page;
        if (!firstPageId) return page;

        let changed = false;
        const nextItems = page.items.map((item) => {
            if (!item) return item;
            if (item.type !== "back") return item;
            if (item.refId && item.refId.trim()) return item;
            changed = true;
    ...
    private applyBackRulesForPage(page: WebDeckPage, firstPageId: string | null) {
        if (!firstPageId) return page;
    ...
    private ensureBootstrapPage() {
        const db = this.getDatabase();
        const countRow = db.prepare("SELECT COUNT(*) as total FROM webdeck_pages").get() as { total: number };
        ...
        `).run(
            ...
            "Página Inicial",
            null,
            WebDeckService.DEFAULT_COLS,
            WebDeckService.DEFAULT_ROWS,
            JSON.stringify(new Array(slots).fill(null)),
            0,
            now,
            now
        );
    ...
    public listPages() {
        ...
        const db = this.getDatabase();
        const rows = db.prepare("SELECT * FROM webdeck_pages ORDER BY position ASC, created_at ASC").all() as any[];
    ...
    public createPage(payload: { name: string; iconSource?: string | null; gridCols?: number; gridRows?: number }) {
        const db = this.getDatabase();
        const now = Date.now();
        const maxPositionRow = db.prepare("SELECT MAX(position) as max_position FROM webdeck_pages").get() as { max_position: number | null };
        const id = randomUUID();
        const name = String(payload.name ?? "").trim() || "Nova Página";
        ...
        logsService.log("webdeck", "page.create", { id, name, gridCols, gridRows });
        return this.findPage(id);
    }

    public findPage(id: string) {
        const db = this.getDatabase();
        const row = db.prepare("SELECT * FROM webdeck_pages WHERE id = ?").get(id) as any;
        if (!row) return null;
    ...
    public updatePage(payload: { id: string; name?: string; iconSource?: string | null }) {
        ...
        this.notifyChange("pages-changed")
        logsService.log("webdeck", "page.update", { id: current.id, name: nextName });
    ...
    public deletePage(id: string) {
        ...
        this.notifyChange("pages-changed")
        logsService.log("webdeck", "page.delete", { id });
        return true;
    }

    public setGrid(pageId: string, gridCols: number, gridRows: number) {
        ...
        const overflowItems = current.items.slice(nextSlots).filter((item): item is WebDeckItem => Boolean(item));
        ...
        this.notifyChange("pages-changed")
        logsService.log("webdeck", "page.grid", { id: current.id, gridCols: nextCols, gridRows: nextRows });
    ...
    public removeItem(pageId: string, index: number) {
        ...
        this.notifyChange("pages-changed")
        logsService.log("webdeck", "item.remove", { pageId, index: safeIndex });
        return this.findPage(pageId);
    }

    public moveItem(pageId: string, fromIndex: number, toIndex: number) {
        const current = this.findPage(pageId);
        if (!current) return null;
        const from = Math.trunc(fromIndex);
        const to = Math.trunc(toIndex);
        if (!Number.isFinite(from) || !Number.isFinite(to)) return null;
        if (from < 0 || from >= current.items.length) return null;
        if (to < 0 || to >= current.items.length) return null;
        ...
        this.persistPage({ ...safePage, updatedAt: Date.now() });
        this.notifyChange("pages-changed")
        logsService.log("webdeck", "item.move", { pageId, from, to });
        return this.findPage(pageId);
    }

    public listAutoIcons(): WebDeckAutoIcons {
        const db = this.getDatabase();
        const rows = db
            .prepare("SELECT kind, icon_key, icon FROM webdeck_auto_icons")
            .all() as Array<{ kind: string; icon_key: string; icon: string | null }>;
    ...
    public setAutoItemIcon(itemKey: string, iconSource: string | null | undefined): WebDeckAutoIcons {
        const key = String(itemKey ?? "").trim();
        if (!key) return this.listAutoIcons();
        const db = this.getDatabase();
        const current = db
            .prepare("SELECT icon FROM webdeck_auto_icons WHERE kind = ? AND icon_key = ? LIMIT 1")
            .get("item", key) as { icon: string | null } | undefined;
