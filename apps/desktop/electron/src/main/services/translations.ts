...
export class TranslationService {
  private builtinMessages: Record<string, Record<string, string>> = {
    "en-US": {
      ...
      "updates.loading.downloading": "Downloading update",
      "updates.loading.installing": "Installing update. The app may take a while to reopen.",
    ...
    "pt-BR": {
      ...
      "tray.exit": "Sair",
      "updates.loading.checking": "Procurando atualizações",
      "updates.loading.downloading": "Baixando atualização",
      "updates.loading.installing": "Instalando atualização.\nO aplicativo pode demorar para abrir novamente.",
      "updates.loading.loadingApp": "Carregando aplicativo",
      "updates.notification.title": "Atualização disponível",
      "updates.notification.availableBody": "Nova versão {version} disponível.",
      "updates.notification.installingBody": "Instalando versão {version}.",
    },
  };
  ...
  importLocaleFile(sourcePath: string) {
    ...
    if (!locale) {
      throw new Error("Locale inválido no arquivo de tradução.");
    }

    const name = payload.name || locale;
    const messages = payload.messages ?? {};
    if (!messages || typeof messages !== "object") {
      throw new Error("Mensagens inválidas no arquivo de tradução.");
    }
