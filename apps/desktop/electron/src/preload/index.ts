...
type WebPageShortcutResult = import("../types/webpages.js").WebPageShortcutResult;
...
const globalObserverListeners = new Set<(payload: ObserverEventPayload) => void>();
let globalObserverSubscribed = false;
const globalObserverEventHandler = (_event: unknown, payload: ObserverEventPayload) => {
  globalObserverListeners.forEach((listener) => {
    listener(payload);
  });
};
...
type AppsChangedPayload = { type: string; data?: unknown; timestamp: number };
const appsChangedListeners = new Set<(payload: AppsChangedPayload) => void>();
let appsChangedSubscribed = false;
const appsChangedHandler = (_event: unknown, payload: AppsChangedPayload) => {
  appsChangedListeners.forEach((listener) => {
    listener(payload);
  });
};
...
const underdeckApi: UnderDeckApi = {
  system: {
    getDeviceInfo: () => ipcRenderer.invoke("SystemSV-GetDeviceInfo"),
    makeQrCodeDataUrl: (text) => ipcRenderer.invoke("SystemSV-MakeQrCodeDataUrl", text),
  },
  i18n: {
  ...
  apps: {
    ...
    onChanged: (listener) => {
      ...
      if (!appsChangedSubscribed) {
        ipcRenderer.on("AppsSV-Changed", appsChangedHandler);
        appsChangedSubscribed = true;
      }
      return () => {
        appsChangedListeners.delete(listener);
        if (appsChangedListeners.size === 0 && appsChangedSubscribed) {
          ipcRenderer.removeListener("AppsSV-Changed", appsChangedHandler);
          appsChangedSubscribed = false;
        }
      };
    },
  },
  ...
  globalObserver: {
    ...
    subscribe: (listener) => {
      ...
      return () => {
        underdeckApi.globalObserver.removeListener(listener);
      };
    },
    removeListener: (listener) => {
      globalObserverListeners.delete(listener);
      if (globalObserverListeners.size === 0 && globalObserverSubscribed) {
        ipcRenderer.removeListener("GlobalObserverSV-Event", globalObserverEventHandler);
        globalObserverSubscribed = false;
      }
    },
  },
  overlay: {
  ...
  logs: {
    getSettings: (...args) => ipcRenderer.invoke("LogsSV-GetSettings", ...args),
    setSettings: (...args) => ipcRenderer.invoke("LogsSV-SetSettings", ...args),
    openLogFile: (...args) => ipcRenderer.invoke("LogsSV-OpenLogFile", ...args),
    clearLogFile: (...args) => ipcRenderer.invoke("LogsSV-ClearLogFile", ...args),
    clearLogs: (...args) => ipcRenderer.invoke("LogsSV-ClearLogs", ...args),
  },
  windowControls: {
  ...
  media: {
    ...
    readAsDataUrl: (source) => ipcRenderer.invoke("MediaSV-ReadAsDataUrl", source),
    getFileSize: (source) => ipcRenderer.invoke("MediaSV-GetFileSize", source),
  },
  theme: {
  ...
  obs: {
    ...
    setInputVolume: (inputNameOrUuid, inputVolumeMul) => ipcRenderer.invoke("ObsSV-SetInputVolume", inputNameOrUuid, inputVolumeMul),
