...
const globalObserverListeners = new Set();
let globalObserverSubscribed = false;
const globalObserverEventHandler = (_event, payload) => {
...
const updateStateHandler = (_event, payload) => {
  updateStateListeners.forEach((listener) => {
    listener(payload);
  });
};

ipcRenderer.on("UpdatesSV-DebugLog", (_event, payload) => {
  try {
    const data = payload || {};
...
const underdeckApi = {
  ...
  obs: {
    ...
    setInputVolume: (inputNameOrUuid, inputVolumeDb) => ipcRenderer.invoke("ObsSV-SetInputVolume", inputNameOrUuid, inputVolumeDb),
